...
async def search_with_query_param(
    body: dict = Body(...),
    provider: str = Query(None)  # 添加URL查询参数
):
    """通过查询参数执行向量搜索"""
    try:
        # 从请求体中提取参数
        ...
        logger.info(f"Search request with query param - Raw Provider: {provider_str}, Query: {query}, Collection: {collection_id}, Top K: {top_k}, Threshold: {threshold}, Word Count Threshold: {word_count_threshold}")
        ...
        valid_providers = [VectorDBProvider.MILVUS.value.lower(), VectorDBProvider.CHROMA.value.lower()]
        if provider_str not in valid_providers:
            logger.warning(f"Invalid provider: '{provider_str}', valid options are: {valid_providers}")
...
async def search_with_path_param(
    provider: str,
    body: dict = Body(...)
):
    """通过路径参数执行向量搜索"""
    try:
        ...
        # Log the incoming search request details
        logger.info(f"Search request with path param - Raw Provider: {provider}, Query: {query}, Collection: {collection_id}, Top K: {top_k}, Threshold: {threshold}, Word Count Threshold: {word_count_threshold}")
        ...
        if provider_str not in [VectorDBProvider.MILVUS.value, VectorDBProvider.CHROMA.value]:
